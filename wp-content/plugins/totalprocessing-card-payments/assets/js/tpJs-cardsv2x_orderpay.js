( function ( tp, $ ) {
    ...
    function instantiateCheckoutIdOrder(){
        ...
        let checkout_form = jQuery( 'form#order_review' );
        let checkoutData  = checkout_form.serialize();
        if(tp.tpCardVars.slickOneClick === '1'){
        ...
        checkoutData += '&action=' + globalPrefix + 'tpOrderPay';
    ...
    function completeOrderTpCards(endpoint,checkoutData){
        ...
        $.ajax({
            ...
            success: function(response){
                cardsv2Log(response);
                $( 'form#order_review' ).removeClass('prociessing');
    ...
    function validate_tp_cardsv2_checkout(args){
        ...
        Promise.resolve(
            $.ajax({
                type: 'POST',
                url: tp.tpCardVars.adminUrl,
                data: { action: globalPrefix + 'validate_tp_cardsv2_checkout', resourcePath: args[0], order_id: args[1], from_order_pay: 1},
                success: function(response) {
                    cardsv2Log(response);
    ...
    function do_log_errors(args){
        Promise.resolve(
            $.ajax({
                ...
                error: function(jqXHR, textStatus, errorThrown){
                    alert( "Error(#3):" + 'Message::' + textStatus + '->' + errorThrown + "\n" + 'resource:' + args[0] );  
                    cardsv2Log(jqXHR);
                    cardsv2Log(textStatus);
                    cardsv2Log(errorThrown);
                },
    ...
    tp.tpCardsHandoff = function(e) {
        console.log('init tp.tpCardsHandoff');
        console.log(wc_checkout_params.checkout_url);
        if($( 'form#order_review' ).find('input[name^="payment_method"]:checked').val() !== tp.tpCardVars.pluginId){
            return;
        }
        console.log('processing form');
        console.log(e);
        let chekoutForm  = $( 'form#order_review' );
        let checkoutData = chekoutForm.serialize();
        chekoutForm.addClass('processing');
        var payOrderEndpoint   = window.location;
        completeOrderTpCards( payOrderEndpoint, checkoutData );
        return false;
    };
...
jQuery( function(){
    ...
    jQuery(document.body).on('payment_method_selected', function() {
        var selectedPaymentMethod = jQuery( '.woocommerce-checkout input[name="payment_method"]:checked' ).val();
        if( selectedPaymentMethod == tpGlobalVars.pluginId ){
            console.log(tpGlobalVars.pluginId + '::' + selectedPaymentMethod);
            window.wc_gateway_tp.initCheckoutIdOrder();
	}
    });
    
    var checkout_form = jQuery( 'form#order_review' );
    checkout_form.on( 'submit', window.wc_gateway_tp.tpCardsHandoff );
});
