( function ( tp, $ ) {
    ...
    function fetchOrderTpCardsModalVersion(endpoint,checkoutData){
        ...
        fetch(endpoint , {
            ...
            if(json.hasOwnProperty('messages')){
                jQuery('form#order_review').prepend('<div class="woocommerce-NoticeGroup woocommerce-NoticeGroup-checkout">' + json.messages + '</div>');
            }
            return json;
        }).then(function(json){
            ...
            oppwajs.setAttribute('src', 'https://' + tpCardVars.platformBase + '/v1/paymentWidgets.js?checkoutId=' + json.data.checkoutId);
    ...
    tp.tpCardsHandoff = function() {
        if(jQuery('form#order_review').find('input[name^="payment_method"]:checked').val() === tpCardVars.pluginId){
            ...
            fetchOrderTpCardsModalVersion(tpCardVars.adminUrl,checkoutData);
            return false;
        }
        return;
    };
...
jQuery(function(){
    ...
    checkout_form.on( 'submit', wc_gateway_tp.tpCardsHandoff );
    ...
    if(jQuery('ul.woocommerce-error').length){
        ...
        var tpNotices = document.querySelector("ul.woocommerce-error");
        tpNotices.scrollIntoView();
    }
});
