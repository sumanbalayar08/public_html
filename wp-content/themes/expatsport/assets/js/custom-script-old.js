...
function initScrollTriggerScenes() {
   ...
   /*gsap.fromTo('.img-dots', 
      {
        opacity: 0 // Initial opacity
      }, 
      {
        opacity: 0.02, // Final opacity
        duration: 1, // Duration of the animation to reach 0.1
        repeat: -1, // Infinite repeat
        repeatDelay:5,
        yoyo: true, // Reverses animation back to the start
        ease: 'power1.inOut', // Easing function for smooth animation
      }
    );

   gsap.fromTo('.fade-img', 
      {
      ...
      }, 
      {
        ...
        repeatDelay:5,
        //yoyo: true, // Reverse animation back to the start
      }
    );*/
...
tl.fromTo('.img-dots', 
  ...
  }, 
  {
    ...
    var triggerKPIBGElement = $('.empty-sec--kpi').get(0);
    var timelineKPIBGDiv = gsap.timeline({
        scrollTrigger: {
            trigger: triggerKPIBGElement,
            start: "20% bottom",
            end: "top 30%",
            scrub: true,
        },
    });
    timelineKPIBGDiv.fromTo(
        $('.section__bg--events-kpi').get(0), 
